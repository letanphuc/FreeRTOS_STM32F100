...
void nrf24l01_initialize_debug(bool rx, unsigned char p0_payload_width, bool enable_auto_ack)
{
	...
	unsigned char txIP[5] = { 192, 168, 1, 1, 111 };
	unsigned char rxIP[5] = { 192, 168, 1, 1, 1 };
...
{
	if(GPIO_ReadOutputDataBit(CE_PORT, CE_PIN) != 0)
		return true;
	else
		return false;	
}
...
{
	if(GPIO_ReadOutputDataBit(CSN_PORT, CSN_PIN) != 0)
		return true;
	else
		return false;	
}
